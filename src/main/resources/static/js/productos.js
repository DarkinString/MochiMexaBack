/* Fuente compartida de productos. El catálogo base evita una pantalla vacía
 * durante la primera carga; en cuanto responde la API, el servidor es la fuente
 * de IDs, precios, stock, categorías e imágenes. */
(() => {
    ...
    let remotos = null;
    let categoriasRemotas = [];
    let cargaActiva = null;
    let ultimoError = null;
    const categoria = valor => ({ pockys: 'poky', pocky: 'poky', pokys: 'poky', 'otros snacks': 'snacks', snack: 'snacks', snacks: 'snacks' }[Mochi.normalizar(valor)] || Mochi.normalizar(valor));
    const categoriaLegada = valor => Mochi.normalizar(valor) === 'snacks' ? 'poky' : categoria(valor);
    ...
    const imagenParaApi = valor => {
        if (String(valor).startsWith('data:')) return valor;
        try {
            const url = new URL(valor, location.href);
            const marker = '/frontend/assets/';
            const index = url.pathname.indexOf(marker);
            return url.origin === location.origin && index >= 0 ? `../assets/${decodeURIComponent(url.pathname.slice(index + marker.length))}` : url.href;
        } catch { return valor; }
    };
    ...
    function listar() {
        if (Array.isArray(remotos)) return remotos.map(p => ({ ...p }));
        const lista = bases.map(p => ({ ...p }));
        listaGuardada('catalogoProductos').forEach((p, indice) => lista.push({
            ...p, id: `local-${p.id || indice}`, categoria: p.versionCatalogo === 2 ? p.categoria : categoriaLegada(p.categoria), sku: p.sku || `LOCAL-${p.id || indice}`
        }));
    ...
    function mapearProducto(producto) {
        const slug = producto.slug || String(producto.idProducto);
        const base = bases.find(item => item.id === slug) || bases.find(item => Mochi.normalizar(item.nombre) === Mochi.normalizar(producto.nombre));
        const stock = Number(producto.stock);
        const activo = producto.activo !== false;
        return {
            id: slug,
            backendId: Number(producto.idProducto),
            nombre: producto.nombre,
            descripcion: producto.descripcion || '',
            precio: Number(producto.precio),
            stock: Number.isInteger(stock) ? stock : 0,
            categoria: categoria(producto.categoria?.nombre || base?.categoria || ''),
            categoriaId: producto.categoria?.idCategoria,
            imagen: Mochi.imagenSegura(producto.imagen || base?.imagen),
            badge: stock === 0 || !activo ? 'AGOTADO' : (producto.badge || base?.badge || 'EN STOCK'),
            sku: producto.slug?.toUpperCase() || `PROD-${producto.idProducto}`,
            marca: producto.marca || '',
            estado: activo && stock > 0 ? 'Activo' : 'Inactivo',
            activo
        };
    }

    async function cargar(forzar = false) {
        if (cargaActiva && !forzar) return cargaActiva;
        cargaActiva = Promise.all([
            Mochi.api.get('/api/products'),
            Mochi.api.get('/api/categories')
        ]).then(([productos, categoriasApi]) => {
            if (!Array.isArray(productos) || !Array.isArray(categoriasApi)) throw new Error('El catálogo recibido no tiene el formato esperado.');
            categoriasRemotas = categoriasApi;
            remotos = productos.map(mapearProducto).filter(p => Number.isFinite(p.precio) && p.precio > 0);
            ultimoError = null;
            avisar();
            return listar();
        }).catch(error => {
            ultimoError = error;
            throw error;
        }).finally(() => { cargaActiva = null; });
        return cargaActiva;
    }

    async function idCategoria(valor) {
        if (!categoriasRemotas.length) await cargar();
        const buscada = categoria(valor);
        const encontrada = categoriasRemotas.find(item => categoria(item.nombre) === buscada);
        if (!encontrada) throw new Error('La categoría no existe en el servidor.');
        return encontrada.idCategoria;
    }

    function editar(id, datos) {
        const actual = obtener(id);
        if (!actual) throw new Error('El producto ya no está disponible. Vuelve al catálogo administrativo.');
        const siguiente = validar(datos);
        if (!Mochi.leer('mochiAuth', null)?.token) {
            if (!Mochi.guardar(cambiosClave, { ...leerCambios(), [id]: siguiente })) return false;
            avisar();
            return true;
        }
        if (!actual.backendId) throw new Error('El producto aún no está disponible en el servidor. Recarga la página.');
        return editarRemoto(actual, siguiente);
    }

    async function editarRemoto(actual, siguiente) {
        const guardado = await Mochi.api.put(`/api/products/${actual.backendId}`, {
            nombre: siguiente.nombre, slug: actual.id, descripcion: siguiente.descripcion,
            precio: siguiente.precio, stock: siguiente.stock, marca: actual.marca || 'MochiMexa',
            imagen: imagenParaApi(siguiente.imagen), badge: siguiente.stock === 0 ? 'AGOTADO' : 'EN STOCK',
            activo: siguiente.stock > 0, idCategoria: await idCategoria(siguiente.categoria)
        });
        remotos = remotos.map(item => item.id === actual.id ? mapearProducto(guardado) : item);
        avisar();
        return true;
    }
    ...
    function registrar(datos) {
        ...
        return registrarRemoto(siguiente);
    }

    async function registrarRemoto(siguiente) {
        const guardado = await Mochi.api.post('/api/products', {
            nombre: siguiente.nombre, descripcion: siguiente.descripcion, precio: siguiente.precio,
            stock: siguiente.stock, marca: 'MochiMexa', imagen: imagenParaApi(siguiente.imagen),
            badge: siguiente.stock === 0 ? 'AGOTADO' : 'EN STOCK', activo: siguiente.stock > 0,
            idCategoria: await idCategoria(siguiente.categoria)
        });
        const mapped = mapearProducto(guardado);
        remotos = [...(remotos || []), mapped];
        avisar();
        return mapped.id;
    }
    ...
    Mochi.productos = { listar, obtener, encontrar, editar, registrar, cargar, get error() { return ultimoError; } };
    Mochi.productos.listo = Mochi.api.activa && typeof fetch === 'function'
        ? cargar().catch(error => { console.error('No se pudo cargar el catálogo del backend:', error); return listar(); })
        : Promise.resolve(listar());
})();
