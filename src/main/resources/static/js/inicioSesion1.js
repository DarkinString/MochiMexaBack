...
// La contraseña se envía únicamente al endpoint de autenticación por HTTPS.
document.getElementById('login-form').addEventListener('submit', async function (event) {
    ...
    try {
        if (!Mochi.api.activa) {
            ...
            if (usuario && !usuario.claveHash) return alert('Esta cuenta antigua no tiene una credencial compatible.');
            if (!usuario || await Mochi.derivarClave(clave, usuario.sal) !== usuario.claveHash) return alert('Correo o contraseña incorrectos. Inténtalo de nuevo.');
        ...
        if (Mochi.iniciarSesion(autenticacion)) window.location.href = Mochi.destinoSesion();
    } catch (error) {
        console.error('No se pudo iniciar sesión:', error);
        alert(error.status === 401 ? 'Correo o contraseña incorrectos. Inténtalo de nuevo.' : error.message);
