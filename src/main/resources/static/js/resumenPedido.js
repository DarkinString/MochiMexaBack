document.addEventListener('DOMContentLoaded', async () => {
    ...
    formDireccion.addEventListener('submit', async event => {
        ...
        try {
            ...
            const id = await tienda.guardarDireccion(datos);
            if (!id) return;
    ...
    $('formPago').addEventListener('submit', async event => {
        ...
        try {
            ...
            const resultado = await tienda.crearPedido({ direccionId: $('direccionPedido').value, metodo: document.querySelector('input[name="metodo"]:checked')?.value, codigo, solicitudId: intento.id, firma: resumen.firma });
            if (!resultado) return;
    ...
    window.addEventListener('pageshow', async () => {
        if (completado) {
            ...
            if (Mochi.leer('mochiAuth', null)?.token) await tienda.cargarCuentaRemota();
            await Cart.recargar(); cargarDirecciones(); actualizarResumen();
        }
    });
    await Promise.all([Mochi.productos.listo, tienda.listo, Cart.listo]);
