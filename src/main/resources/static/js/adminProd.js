...
document.addEventListener('DOMContentLoaded', async () => {
    if (Mochi.api.activa && !Mochi.esAdmin()) {
        ...
        location.href = Mochi.usuarioActual() ? Mochi.ruta('index.html') : Mochi.ruta('iniciaSesion.html');
        return;
    }
    if (Mochi.leer('mochiAuth', null)?.token) await Mochi.productos.listo;
    ...
    form.addEventListener('submit', async event => {
        ...
        try {
            const guardado = await (original ? Mochi.productos.editar(idEditar, datos) : Mochi.productos.registrar(datos));
            if (!guardado) return;
            if (original) original = Mochi.productos.obtener(idEditar);
            else form.reset();
            alerta(original ? 'Cambios guardados en el servidor. El artículo mantiene el mismo ID en todas las vistas.' : 'Producto registrado en el servidor y disponible en el catálogo.', 'success');
