document.addEventListener('DOMContentLoaded', async () => {
    ...
    function pintarProducto() {
        ...
        $('productoStock').textContent = agotado ? 'Agotado por el momento' : producto.stock == null ? 'Disponibilidad final por confirmar con la tienda.' : `${producto.stock} unidades disponibles`;
        ['comprarProducto', 'agregarProducto', 'masProducto', 'menosProducto', 'cantidadProducto'].forEach(k => { $(k).disabled = agotado; });
    ...
    async function comprar(irAlPago) {
        try {
            if (!producto || !await Cart.agregarProducto(id, cantidadValida())) return;
            if (irAlPago) await Cart.pagar();
    ...
    $('formResena').addEventListener('submit', async event => {
        ...
        try {
            ...
            if (!await tienda.guardarResena(id, $('estrellasResena').value, $('comentarioResena').value)) return;
            $('formResena').hidden = true; pintarResenas();
        } catch (error) { aviso('estadoResena', error.message, true); }
    });
    $('listaResenas').addEventListener('click', async event => {
        if (event.target.closest('[data-eliminar-resena]') && confirm('¿Eliminar tu reseña de este producto?')) {
            try { if (await tienda.eliminarResena(id)) pintarResenas(); }
            catch (error) { aviso('estadoResena', error.message, true); }
        }
    });
    ...
    await Promise.all([Mochi.productos.listo, Cart.listo]);
    pintarProducto();
    try { await tienda.cargarResenas(id); } catch (error) { aviso('estadoResena', error.message, true); }
});
