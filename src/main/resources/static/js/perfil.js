/* Perfil conectado a la cuenta autenticada del backend. */
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try { await tienda.listo; } catch (error) { estado('estadoPerfil', error.message, true); return; }
    ...
    function pintarPedidos() {
        ...
        $('listaPedidosPerfil').innerHTML = (todosPedidos ? pedidos : pedidos.slice(0, 3)).map(p => `
            <details class="perfil-pedido"><summary><span><strong>Pedido #${escapar(p.id.slice(0, 11).toUpperCase())}</strong><small>${escapar(p.fecha)} · ${p.items?.reduce((s, i) => s + i.cantidad, 0) || 0} artículos</small></span><span><strong>${tienda.dinero(p.total)} MXN</strong><span class="tienda-etiqueta">${escapar(p.estado)} · Sin cobrar</span></span></summary>
                ...
                <p class="tienda-nota">Envío: ${tienda.dinero(p.costoEnvio || 0)} · Método de prueba: ${escapar(p.metodoPago)}<br>Pedido guardado en el servidor; no implica un cobro real.</p>
    ...
    $('formPerfil').addEventListener('submit', async event => {
        ...
        try {
            if (!await tienda.actualizarPerfil({ nombre: $('perfilNombreInput').value, email: $('perfilEmail').value, telefono: $('perfilTelefono').value, foto })) return;
            cargarPerfil();
            estado('estadoPerfil', 'Tus datos y foto se guardaron en tu cuenta.');
    ...
    $('listaDirecciones').addEventListener('click', async event => {
        ...
        if (eliminar && comprobarSesion() && confirm('¿Eliminar esta dirección de tu cuenta?')) {
            if (await tienda.eliminarDireccion(eliminar.dataset.direccionEliminar)) pintarDirecciones();
        }
    });
    formDireccion.addEventListener('submit', async event => {
        ...
        try {
            ...
            if (!await tienda.guardarDireccion({ ...datos, principal: campo('principal').checked }, editandoDireccion)) return;
    ...
    $('formClavePerfil').addEventListener('submit', async event => {
        ...
        try {
            ...
            $('formClavePerfil').reset(); estado('estadoClavePerfil', 'Contraseña actualizada.');
    ...
    window.addEventListener('pageshow', async () => { if (Mochi.leer('mochiAuth', null)?.token) await tienda.cargarCuentaRemota(); cargarPerfil(); });
