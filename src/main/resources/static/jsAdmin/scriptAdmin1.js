...
async function obtenerPedidos() {
    try {
        if (Mochi.leer('mochiAuth', null)?.token) {
            const datos = await Mochi.api.get('/api/admin/orders');
            state.pedidos = datos.map(Mochi.tienda.mapearPedido);
...
async function actualizarPedido(id, datosActualizados) {
    ...
    if (Mochi.leer('mochiAuth', null)?.token) {
        try {
            const remoto = await Mochi.api.patch(`/api/admin/orders/${actual.backendId}/status`, { estado: datosActualizados.estado });
            const actualizado = Mochi.tienda.mapearPedido(remoto);
            state.pedidos = state.pedidos.map(p => p.id === id ? actualizado : p);
        } catch (error) {
...
async function obtenerClientes() {
    try {
        if (Mochi.leer('mochiAuth', null)?.token) {
            const usuarios = await Mochi.api.get('/api/admin/users');
            state.clientes = usuarios.filter(user => String(user.rol).toUpperCase() !== 'ADMIN').map(user => {
                const perfil = Mochi.api.mapearUsuario(user);
                const pedidos = state.pedidos.filter(order => order.usuarioId === perfil.id);
                const ultimo = pedidos.map(order => Date.parse(order.creadoEn)).filter(Number.isFinite).sort((a, b) => b - a)[0];
                return { id: perfil.id, nombre: perfil.nombre, email: perfil.email, ubicacion: pedidos[0]?.ubicacion || 'Sin dirección', pedidos: pedidos.length,
                    ultimaActividad: ultimo ? new Date(ultimo).toLocaleDateString('es-MX') : 'Sin pedidos', avatar: perfil.foto || '',
                    iniciales: perfil.nombre.split(/\s+/).map(part => part[0]).join('').slice(0, 2).toUpperCase() };
            });
...
function guardarConfiguracion() {
    if (Mochi.leer('mochiAuth', null)?.token) return Mochi.tienda.guardarConfiguracionRemota(state.configuracion);
...
function cargarConfiguracion() {
    const datos = Mochi.leer('mochiAuth', null)?.token ? Mochi.tienda.configuracionActual() : Mochi.leer('mochimexa_configuracion', {});
...
async function guardarAjustesDesdeUI() {
    ...
    try {
        if (!await guardarConfiguracion()) return;
        mostrarToast(Mochi.leer('mochiAuth', null)?.token ? "Preferencias guardadas en el servidor." : "Preferencias guardadas en este navegador. No se activan cobros ni correos automáticos.", "success");
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        if (Mochi.api.activa && !Mochi.usuarioActual()) {
            location.href = Mochi.ruta('iniciaSesion.html');
            return;
        }
        if (Mochi.api.activa && !Mochi.esAdmin()) {
            alert('Esta sección requiere una cuenta administradora.');
            location.href = Mochi.ruta('index.html');
            return;
        }
        await Mochi.tienda.listo;
        initNavegacion();
